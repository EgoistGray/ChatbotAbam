...
const Database = require('./postgres-database.js');
...
class Encharten {
    constructor() {
        ...
        this.noWork = "Kayaknya gak ada deh 😊";
        this.notComplete = "Kurang lengkap pak..";
        this.messageHeaderOld = `[JADWAL PH & REMEDIAL] \n\nBerikut jadwal PH & Remedial untuk minggu depan. Jadwalnya bisa di save ya temen-temen :)\n\nInfo:\n`;
        this.messageHeader = `[INFO & PENUGASAN]\n${this._getTommorowDate()}\n `;
        this.messageFooter = `\nnote: Jika ada tambahan bisa langsung tambahin aja atau chat abam ya.\nsekretarismu♥`
        // this.messageFooter = `\nsekretarismu♥`
        ...
        this.database.selectLatestDatabase().then(unparsedDatabase => {
            console.log('UNPARSED', unparsedDatabase);
            this.unparsedDatabase = unparsedDatabase;
            if (this.unparsedDatabase === '') {
                this._setupNewDatabase();
    ...
    _updateDatabase(newDatabase) {
        ...
        this.database.updateDatabase(JSON.stringify(this.currentDatabase));
    }
    ...
    _getTommorowDate(){
        ...
        tommorow.setDate( today.getDate() + increment);
        today = undefined;
